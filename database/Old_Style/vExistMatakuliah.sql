create View vExistMK as SELECT
...
WHERE
a.id = b.tm_tahun_ajaran_id AND b.id = c.tm_semester_id AND c.id = d.tr_matakuliah_semester_prodi_id AND a.is_aktif = 1 AND c.tm_matakuliah_id=e.id AND d.tm_staff_id = f.id
